...
unit Statistics;         //agenda topic for EKON 29
...
///https://github.com/mikerabat/mrmath/blob/master/MatrixConst.pas
type
  TMeanVarRec = record
     aMean : double;
     aVar : double;
...
function _Factorial(n : integer) : Int64;
...
var p : Array[0..7] of double;
    q : Array[0..7] of double;
    c : Array[0..6] of double;
    
 const    spi = 0.9189385332046727417803297;
...
var cof : Array[0..5] of double; // = (76.18009172947146, -86.50532032941677,
...
begin
     ...
     if m > cITMax then
        xraise (Exception.Create('a or b too big or cMaxIt too small in betacf'));
...
begin
     if (x < 0) or (x > 1) then
        xraise (Exception.Create('Bad x in beatI'));
...
begin
     if (x < 0) or (a <= 0) then
        xraise (Exception.Create('Invalid arguments for gammap'));
...
initialization
  ...
  writ(int64tostr(_Factorial(14)));
  writ(flots(FactorialS(14)));
  writ(flots(Fact(14)));
  writ((bigFac(14)));
   writ(itoa(_binom(46,6)));
  writ(flots(BinomialCoeff(46,6))); 
