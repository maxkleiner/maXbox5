...
type
  TQuickSortFunc = function(const Item1, Item2: integer) : integer;


function DoubleSortFunc(const Item1, Item2: integer) : integer;
...
//procedure QuickSort0(A : PConstDoubleArr; Width : integer); //overload;
...
function findRoot1( func : TRootFuncWithDerrive; out root : double; x1, x2 : double; xAcc : double {= 1e-8} ) : boolean; //overload;
...
var fMt : Array of longint; //int64; //LongWord;  // array of the state vector
    fmag01 : Array[0..1] of int64; //LongWord;
...
procedure TRandomGeneratorInitMersenne(Seed: int64);
var mti : longint; //int64;
begin
     if seed = 0 then
...
function _binom(n, k : integer) : int64;
var tmp : int64;
    i : integer;
begin
     if k = 0 then
...
initialization
  ...
  writ(flots(pow(2,64-1))) ;
  writ(bigpow(2,64-1));
  //-2 147 483 648  ..  2 147 483 647
  writ(flots(pow(2,32-1)-1)) ;
  writ(bigpow(2,32-1));
  maxcalcF('2^(32-1)-1-2*(2^(32-1))+1')
  maxcalcF('2^(32-1)-2*(2^(32-1))')
  writ(flots(pow(2,32-1)-2*pow(2,32-1))) ;
  maxcalcF('-10*5')
  //IntPowerInt( Value : Int64; Exponent : Int) : Int64;
  //-9 223 372 036 854 775 808 .. +9 223 372 036 854 775 807
  writ(int64tostr(IntPowerInt(2,64-1)-1));
  writ(int64tostr(IntPowerInt(2,64-1)-0));
  // writ(flots(pow(2,32-1)+1)) ;
     writ(itoa(_binom(49,6)));
  writ(flots(BinomialCoeff(49,6))); 
   writ(flots(BinomialCoeff(49,6))); 
...
Beim deutschen Lotto 6 aus 49 gibt es 13 983 816 verschiedene Kombinationen von sechs Zahlen, die man aus den 49 möglichen auswählen kann.?

Das ergibt sich mathematisch aus dem Binomialkoeffizienten
